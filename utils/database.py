...
class DatabaseInterface:
    ...
    async def hardreset(self):
        async with self.pool.acquire() as conn:
            await conn.execute(
                "DROP TABLE UserItems; DROP TABLE Recipes; DROP TABLE Bakeries; DROP TABLE Users;"
            )
        self.pool = None
        await self.init()
    ...
    async def create_recipe(
        self, name: str, quantity: int, human: str, value: int, ingredients: str
    ):
        print(name, type(name))
        async with self.pool.acquire() as conn:
            return await conn.execute(
                "INSERT INTO Recipes VALUES ($1, $2, $3, $4, $5);",
                name,
                quantity,
                human,
                value,
    ...
    async def get_recipe(self, name: str):
        async with self.pool.acquire() as conn:
            return await conn.fetchrow("SELECT * FROM Recipes WHERE name = $1;", name)

    async def get_top_bakeries(self):
        async with self.pool.acquire() as conn:
            return await conn.fetchrow(
                "SELECT * FROM Bakeries ORDER BY owned_h LIMIT 25;"
            )
